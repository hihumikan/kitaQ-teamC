...
class Commons:
  ...
  def cookie_reg(self,user_id,session) -> str:
    conn = conn_db()              #ここでDBに接続
    cursor = conn.cursor()
    sql = "INSERT INTO session (user_id,session) VALUES (" + str(user_id) + ",'" + str(session) + "') ON DUPLICATE KEY UPDATE session = VALUES(session);"
    print(sql)
    try:
      cursor.execute(sql)
      conn.commit()
...
class Users:
  ...
  def user_reg(self,user_name,email,password,isParent,description):
    conn = conn_db()
    cursor = conn.cursor()
    sql = "INSERT INTO users (user_name,email,password,description,isParent) VALUES( '"+ user_name + "','" + email + "','" + password + "','" + description + "'," + str(isParent) + ");"
