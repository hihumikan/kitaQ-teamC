...
def get_user(user_id):
    ...
    for post in posts_data:
        if os.path.isfile("static/posts/" + str(post[0]) + ".jpg"):
            post_image = "http://api.kitaq.qqey.net/static/posts/" + str(post[0]) + ".jpg"
        else:
            post_image = None
        posts.append({
        "post_id":post[0],
...
def user_reg():
  ...
  response.set_cookie('session', value=session_id, max_age=max_age, path='/', secure=None, httponly=True)
  return response
...
def cookie_set():
  ...
  # Cookieの設定を行う
  max_age = 60 * 60 * 24 * 30 # 30日有効のセッションを発行
  response.set_cookie('uid', value="1234", max_age=max_age, path='/', secure=None, httponly=True)
